...
public class MyScheduleViewModel extends AndroidViewModel {
  ...
  /**
   * Get the options for the loading of listings.
   *
   * @return FirestoreRecyclerOptions containing the appriopriate options for MyScheduleFragments
   */
  private FirestoreRecyclerOptions<Listing> getRecyclerOptions() {
    Query mQuery = MuggerDatabase.getAllListingsReference(db).orderBy("startTime");
    return ListingUtils.getRecyclerOptions(mQuery);
  }

  public RecyclerView.Adapter<ListingsViewHolder> getRecyclerAdapter(CalendarDay day) {
    if (selectedDay != null && selectedDay.equals(day) && adapter != null) {
      return adapter;
    }
    selectedDay = day;
    adapter = new ListingsFirestoreAdapter(getRecyclerOptions(),
        getApplication().getApplicationContext(), mAuth, db, fcm, getFilter(day));
    ((ListingsFirestoreAdapter) adapter).startListening();
    return adapter;
  }
